...
current_audio.addEventListener('ended', async () => {
    if (isNaN(itemIdStorage[0])) {
        itemIdStorage[0] = 1;
        const res = await axios.get(`http://localhost:3000/songs/?id=${itemIdStorage[0]}`);
        ...
        // Array.from(document.getElementsByClassName('songItem'))[`${indexMusic - 1}`].style.background = "rgb(105, 105, 170, .1)";
        // makeAllPlays();

        // document.getElementsByClassName('playListPlay')[indexMusic - 1].classList.remove('bi-play-circle-fill');
        // document.getElementsByClassName('playListPlay')[indexMusic - 1].classList.add('bi-pause-circle-fill');
